...
public class PlayerListHudMixin {
    ...
    @ModifyConstant(method = "extractRenderState", constant = @Constant(intValue = 13), remap = false)
    ...
    private void replacePingIconWithText(GuiGraphicsExtractor context, int slotWidth, int xo, int yo, PlayerInfo info, CallbackInfo ci) {
        PingViewRender.renderPingText(minecraft, context, slotWidth, xo, yo, info);
        ci.cancel();
    }
}